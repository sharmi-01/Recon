...
try:
    ...
    print(f"[{YELLOW}!{RESET}] EASMmon needs python 3.4 > ro run!")
    sys.exit()

# Args parsing
def arguments():
    
    a = argparse.ArgumentParser(description="EASMmon")
...
def banner():
    print(f"""
{YELLOW}⠀Futher information related to the domain is :)""")
    ...
    if parsing.output:
        ...
        if os.path.isfile(reportPath):
            ...
            with open(reportPath, "w") as f:
                f.write(f"# EASMmon TOOL REPORT FROM {domain.upper()}\n\n")
                f.close()
